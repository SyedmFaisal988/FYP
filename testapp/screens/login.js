...
import { View, Text, StyleSheet, StatusBar, SafeAreaView, Image, TouchableOpacity, KeyboardAvoidingView, Alert, AsyncStorage, Dimensions, ScrollView, Keyboard } from 'react-native';
...
class Login extends Component {
    ...
    handleLogin = async () => {
        ...
        if (response.success) {
            AsyncStorage.setItem('token', JSON.stringify(response.token.replace(/"/g, '')))
                ...
                <SafeAreaView style={{ height: height  }}>
                    ...
                    <View style={{ flex: 0.65 }} >
                        <View style={styles.inputWrapper} >
                            <InputField placeholder={"User Name"} value={username} onChange={(value) => this.setState({ username: value })} >
                                <Envalop />
                            </InputField>
                            <InputField secureTextEntry={true} placeholder={"Password"} value={password} onChange={(value) => this.setState({ password: value })} >
                                <Password />
                            </InputField>
                        </View>
                        <Text style={[styles.text, { textAlign: 'right', paddingRight: 35, marginTop: 10, }]} >
                            Forgot Password ?
                        </Text>
